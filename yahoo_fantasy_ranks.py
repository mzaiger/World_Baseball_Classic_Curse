...
class YahooAuthError(RuntimeError):
    """Raised when Yahoo rejects the request as unauthorized (e.g. 403
    'This application is not authorized to perform this action'), so the
    run fails loudly instead of silently marking every player not_found."""
    pass
...
def api_get(session: OAuth2Session, url: str, params: dict | None = None) -> dict:
    ...
    if resp.status_code != 200:
        if resp.status_code == 403 and "not authorized" in resp.text.lower():
            raise YahooAuthError(
                f"Yahoo rejected the request as unauthorized (HTTP 403).\n"
                f"URL: {resp.url}\n"
                f"Response: {resp.text[:500]}\n"
                f"This usually means the app's access was revoked/expired or the "
                f"account session is invalid — re-authorize and refresh YAHOO_TOKEN."
            )
        return {}
